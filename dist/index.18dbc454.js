...
btnElm.addEventListener("click", ()=>{
    ...
    if (btnElm.textContent === "ADD") fetch(`${API_BASE_URL}/teachers`, {
        method: "POST",
        headers: {
        ...
        else alert("Failed to creat a Teacher!");
    }).catch((err)=>{
...
function loadAllTeachers() {
    ...
    fetch(`${API_BASE_URL}/teachers`).then((res)=>{
        if (res.ok) res.json().then((teacherList)=>teacherList.forEach((teacher)=>createNewRow(teacher)));
        else alert("Falied to load teacher list");
...
tblElm.addEventListener("click", (e)=>{
    if (e.target?.classList.contains("delete")) {
        ...
        fetch(`${API_BASE_URL}/teachers/${teacherId}`, {
            ...
            else alert("Falied to delete Teacher");
        }).catch((err)=>{
            alert("Something went wrong, try again");
        });
    } else if (e.target?.classList.contains("edit")) {
        const teacherId = e.target.closest("tr").id.substring(8);
        fetch(`${API_BASE_URL}/teachers/${teacherId}`).then((res)=>{
            if (res.ok) {
            ...
            } else throw new Error("Failed to fetch teacher data for editing");
        }).catch((err)=>{
...
function updateTeacher() {
    ...
    fetch(`${API_BASE_URL}/teachers/${editId}`, {
        ...
        if (res.ok) loadAllTeachers();
        else throw new Error("Falied edit teacher data");
    }).catch((err)=>{
